 ...
 * @retval 0 on success
 */
static int fill_hid_report(struct hid_report * rpt, buttons_t * data){
...
void main(void)
{
	...
	while (1) {
		/* Poll for data and send interrupts when data changes */
		buttons_t data = {0};
		if ((ret = wii_peripheral_fetch(wii, &data.frame)) != 0){
			LOG_ERR("wii peripheral fetch error: %d", ret);
		}
		else{
			/* Data retrieved. Create a report and check if it is new */
			fill_hid_report(&report, &data);
